import { isPlatformBrowser } from '@angular/common';
import { HttpClient, HttpHeaders } from '@angular/common/http';
import { Inject, Injectable, PLATFORM_ID } from '@angular/core';
import { Observable } from 'rxjs';

@Injectable({
  providedIn: 'root',
})
export class AnalysisCentersService {
  httpOption;
  isBrowser: boolean;
  constructor(
    private http: HttpClient,
    @Inject(PLATFORM_ID) private platformId: Object
  ) {
    this.isBrowser = isPlatformBrowser(this.platformId);

    if (this.isBrowser) {
      this.httpOption = new HttpHeaders({
        // 'Content-Type': 'multipart/form-data',
        Authorization: 'Bearer ' + localStorage.getItem('token'),
        // Authorization:
        //   'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..KhfWbG8AYnidtse-aOD0rqrMJM3grS-cjxM5SKzqacE',
      });
    }
  }
  getAllAnalysisCenters(): Observable<any> {
    return this.http.get<any>(
      'http://graduationbroject.runasp.net/api/AnalysisCenters/GetAllAnalysisCenters'
    );
  }
  createAnalysisCenters(data: any): Observable<any> {
    return this.http.post<any>(
      'http://graduationbroject.runasp.net/api/AnalysisCenters/CreateAnalysisCenter',
      data,
      {
        headers: this.httpOption,
      }
    );
  }
  getAnalysisCenterByID(id: any): Observable<any> {
    return this.http.get<any>(
      `http://graduationbroject.runasp.net/api/AnalysisCenters/GetAnalysisCenterById?id=${id}`,

      {
        headers: this.httpOption,
      }
    );
  }
  updateAnalysisCenter(id: any, imageId: any, body: any): Observable<any> {
    return this.http.put<any>(
      `http://graduationbroject.runasp.net/api/AnalysisCenters/UpdateAnalysisCenter?id=${id}&ImageId=${imageId}`,

      body,
      {
        headers: this.httpOption,
      }
    );
  }
  delete(id: any): Observable<any> {
    return this.http.delete(
      `http://graduationbroject.runasp.net/api/AnalysisCenters/DeleteAnalysisCenter?id=${id}`,
      { headers: this.httpOption }
    );
  }
}
